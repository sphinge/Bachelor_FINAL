//g++ -o cancer cancer_classification.cpp
#include <vector>
#include <cmath>
#include <iostream>
...
#define NEPOCH 200
const double RELU_LEAK = 0.00001;
...
int main()
{
    ...
    vector<int> nNeuronsEachLayer = {30, 10, 2}; // Assuming 30 input features and 2 output classes
    ...
    ifstream test_file("../data/cancer_test.csv");
    string line_test, value_test;

    while (getline(test_file, line_test))
    {
        stringstream ss(line_test); 
        getline(ss, value, ',');

        int test_label = stoi(value);
        vector<double> oneHot(2, 0.0);
        oneHot[test_label] = 1.0;
    ...
    std::ofstream weights_file("cancer_plots/weights.csv");

    std::ofstream loss_file("cancer_plots/loss.csv");
    std::ofstream predict_file("cancer_plots/predictions.csv");
    ...
    for (int i = 0; i < NEPOCH; ++i)
    {
        ...
        cout << (i + 1) << "," << loss << "," << accuracy_i * 100 << endl;
        loss_file << (i + 1) << "," << loss << "," << accuracy_i * 100 << endl;
    ...
    for (size_t i = 0; i < X_test.size(); ++i)
    {
        ...
        predict_file << "(" << predictedLabel << "," << realLabel << "),";  
    }
