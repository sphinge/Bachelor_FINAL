...
string neuronFilename(int iLayer, int iNeuron) {
    return "neuron-" + to_string(iLayer) + "_" + to_string(iNeuron) + ".csv";
}
...
public:
    ...
    // double dy_dz, dz_dx;
vector<double> x;
    Neuron(int _iLayer, int _iNeuron, int _nInputs) 
    : iLayer(_iLayer)
    , iNeuron(_iNeuron)
    , nInputs(_nInputs) {
        std::cout << "    Constructing neuron with " << nInputs << " inputs!" << endl;
        ...
        // create empty file
        ofstream file(neuronFilename(iLayer, iNeuron));
    }
    ...
    void dump_weights() {
        ofstream file(neuronFilename(iLayer, iNeuron), ios::app);
...
public:
    // CHANGED
    Layer(int _iLayer, int _nInputs, int _nNeurons)
    : iLayer(_iLayer)
    , nInputs(_nInputs)
    , nNeurons(_nNeurons)
    {
        std::cout << "  Creating layer with " << nInputs << " inputs, " << nNeurons << " neurons!" << endl;
        for (int i = 0; i < nNeurons; ++i)
            neurons.push_back(Neuron(iLayer, i, nInputs));
    }
...
public:
    ...
    Network(vector<int> nNeuronsEachLayer) {
        std::cout << "🔸 Creating network with " << nNeuronsEachLayer.size() - 1 << " layers!" << endl;
        // first element is nInputs
        layers = vector<Layer>();
        for(int i=0; i < nNeuronsEachLayer.size() - 1; ++i)
            layers.push_back(Layer(i, nNeuronsEachLayer[i], nNeuronsEachLayer[i+1]));
    }
...
int main() {
    ...
    std::cout << "🔸 Training with " << NEPOCH << " epochs!" << endl;
    for (int i = 0; i < NEPOCH; ++i) {
        ...
        if(i % 100 == 0)
            std::cout << "Epoch " << (i+1) << " - MSE: " << epochMSE << endl;
    }
