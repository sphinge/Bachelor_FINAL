...
std::ofstream acc_file("iris_plots/acc.csv");
std::ofstream conf_file("iris_plots/confusion.csv");
...
public:
    ...
    double evaluateModel(SoftmaxRegression& model, vector<vector<double>>& X, vector<double>& y) {
        int correct = 0;
...
int main() {
    ...
    if (file.is_open()) {
        while (getline(file, line)) {
            ...
            if (values.size() == 5) { // 4 features + 1 class label
                y.push_back(static_cast<double>(values.back()));
                values.pop_back();
                X.push_back(values);
            }
        }
    ...
    ifstream file_test("../data/iris_test.csv");
    string line_test;
    
    if (file_test.is_open()) {
        while (getline(file_test, line_test)) {
            vector<double> values_test = split(line_test, ',');
            if (values_test.size() == 5) { // 4 features + 1 class label
                y_test.push_back(static_cast<double>(values_test.back()));
    ...
    SoftmaxRegression model(3 /* num_classes */, 4 /* num_features */, 0.001, 1000);
