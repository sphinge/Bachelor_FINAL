...
const int EXCHANGES = 20; // N is the number of times the exchange happens

int main()
{
    const string training_filename = "data/wine_train.csv";
    const string testing_filename = "data/wine_test.csv";
    ...
    ifstream file(training_filename);
    string line, value;
    ...
    while (getline(file, line))
    {
        ...
        getline(ss, value);
        int label = stoi(value) - 1; // The Wine dataset labels are 1, 2, and 3
    ...
    ifstream test_file(testing_filename);
    ...
    while (getline(test_file, line_test))
    {
        ...
        // Read last value as label and one-hot encode
        getline(ss_test, value_test);
        int label = stoi(value_test) - 1; // The Wine dataset labels are 1, 2, and 3
        vector<float> oneHot(3, 0.0);
        oneHot[label] = 1.0;
    ...
    for (int exchange = 0; exchange < EXCHANGES; ++exchange)
    {
        ...
        Network networkClient({13, 8, 3}); // Initialize with an empty structure. Will be updated by deserialize.
